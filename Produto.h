...
TCelula* PesquisarNome(TLista Lista, TProduto Item) {
    ...
    Aux = Lista.primeiro;
    while(Aux->prox != NULL) {
        if(strcmp(Aux->prox->item.nome, Item.nome) == 0){//isso estava errado
            return Aux;
        }
        Aux = Aux->prox;
    }
...
void Atualizar(TLista *Lista, TProduto Item){ //INFORMAR QUE NÃO ESTOU CONSEGUINDO ATUALIZAR O PRODUTO PELO NOME
    TCelula *Aux1;
    Aux1 = PesquisarNome(*Lista, Item);
    if(Aux1 != NULL){
        printf("\nProduto encontrado %s", Aux1->item.nome);
        printf("\nDigite os novos dados do produto para atualizar:");
        LerProduto (&Aux1->prox->item);//(&Aux1->prox->item);
    } else {
        printf("Produto nao encontrado.");
    }
    
}
void OrdemCrescente(TProduto x, TLista *Lista){ //APRESENTAR AQUI P MAGNO
